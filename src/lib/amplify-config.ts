...
// amplify_outputs.json is generated by `ampx pipeline-deploy` in CI.
// Locally it resolves to amplify_outputs_stub.json via the NormalModuleReplacementPlugin
// in next.config.js — so this import always resolves at build time.
import amplifyconfig from '../../amplify_outputs.json';

try {
  if (amplifyconfig && typeof amplifyconfig === 'object') {
    Amplify.configure(amplifyconfig as Record<string, unknown>);
  }
} catch {
  // Stub config may not have valid Amplify shape; graceful degradation in local/dev.
}
